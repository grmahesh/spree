...
describe Spree::Admin::GreetingcardsController, type: :controller do
  ...
  context "adding properties to a greetingcard" do
    let!(:greetingcard) { create(:greetingcard) }
    specify do
      spree_put :update, :id => greetingcard.to_param, :greetingcard => { :greetingcard_properties_attributes => { "1" => { :property_name => "Foo", :value => "bar" } } }
      expect(flash[:success]).to eq("Greetingcard #{greetingcard.name.inspect} has been successfully updated!")
    end
